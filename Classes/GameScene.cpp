...
//我方数据//////////////////
//牌库
extern vector<Card> myCardLib;
//手牌
extern vector<Card> myHandCard;
//可用手牌
extern vector<Card> myEHandCard;
//场上随从
extern vector<Attendant> myAtt;
//可用随从
extern vector<Attendant> myEAtt;
//本轮水晶
extern int myJams;
//可用水晶
extern int myEJams;
//我方英雄血量
extern int myHeroBlood;
//我方水晶精灵
extern vector<Sprite*> myJamSpr;
//我方英雄血量标签
extern Label* myHL;
//玩家尝试抽牌失败的次数
extern int myDrawWrong;
//玩家进行的回合数
extern int myTurnCount;

//AI数据//////////////////
//牌库
extern vector<Card> AICardLib;
//手牌
extern vector<Card> AIHandCard;
//可用手牌
extern vector<Card> AIEHandCard;
//场上随从
extern vector<Attendant> AIAtt;
//可用随从
extern vector<Attendant> AIEAtt;
//本轮水晶
extern int AIJams;
//可用水晶
extern int AIEJams;
//AI英雄血量
extern int AIHeroBlood;
//AI水晶数组
extern vector<Sprite*> AIJamSpr;
//AI英雄血量标签
extern Label* AIHL;
//AI尝试抽牌失败的次数
extern int AIDrawWrong;
//AI进行的回合数
extern int AITurnCount;

//其他数据
extern bool isHumanTurn;
extern bool gameOver;

extern MenuItemImage *CB;
extern MenuItemImage *ET;
extern Menu *MainMenu;

//出牌选择的菜单
extern vector<Sprite*> ptrB;
//监听出牌
extern EventListenerTouchOneByOne *TouchListener, *MoveAttListener;
//被选中的牌
extern Card pkOne;
extern Sprite* bSelectOne;
extern EventListenerTouchOneByOne *DMTouchListener;

extern vector<Sprite*> canUseSpr;

extern int DMScan;

extern vector<string> AILogs;

extern bool needTouchMove;
extern Attendant *chooseAtt;
extern Vec2 YVec;
...
bool GameScene::init()
{
	myDrawWrong = 0; myTurnCount = 0; AIDrawWrong = 0; AITurnCount = 0; isHumanTurn = false; gameOver = false;
	needTouchMove = false; chooseAtt = nullptr; DMTouchListener = nullptr; bSelectOne = nullptr; DMScan = 0;
	...
	//Load Main Menu
	auto setButton = MenuItemImage::create("UI/backB1.png", "UI/backBP1.png", CC_CALLBACK_1(GameScene::menuCallback, this));
	auto handCardAreaButton = MenuItemImage::create("UI/handCardAreaB.png", "UI/handCardAreaBP.png", CC_CALLBACK_1(GameScene::menuCallback, this));
	auto CardLibButton = MenuItemImage::create("UI/cardLibB.png", "UI/cardLibBP.png", "UI/cardLibBP.png", CC_CALLBACK_1(GameScene::menuCallback, this));
	auto endTurnButton = MenuItemImage::create("UI/endTurnB.png", "UI/endTurnBP.png", "UI/endTurnD.png", CC_CALLBACK_1(GameScene::menuCallback, this));
...
void GameScene::menuCallback(Ref *sender)
{
	...
	case 0:
	{
		...
		auto noButton = MenuItemImage::create("UI/noB.png", "UI/noBP.png", CC_CALLBACK_1(GameScene::menuCallback, this));
		auto yesButton = MenuItemImage::create("UI/yesB.png", "UI/yesBP.png", CC_CALLBACK_1(GameScene::menuCallback, this));
	...
	case 1:
	{
		if (isHumanTurn)
		{
			...
			auto sureButton = MenuItemImage::create("UI/sureB.png", "UI/sureBP.png", CC_CALLBACK_1(GameScene::cardSMenuCallback, this));
			sureButton->setTag(0);
	...
	case 2:
	{
		...
		scheduleOnce(schedule_selector(GameScene::scheduleCallBack2), 1.5);
	}
	break;
...
void GameScene::cardSMenuCallback(cocos2d::Ref * sender)
{
	if (haveSound) { AudioEngine::play2d("Sounds/bi.mp3", false, 0.3); }
...
void GameScene::DMmenuCallback(Ref * sender)
{
...
void GameScene::DMmenuCallback2(Ref *sender)
{
	MainMenu->setEnabled(true);
...
void GameScene::DMmenuCallback3(Ref *sender)
{
	MainMenu->setEnabled(true);
...
void GameScene::initCardGameData()
{
...
void GameScene::gameStart()
{
	...
	scheduleOnce(schedule_selector(GameScene::scheduleCallBack), 3.5);
	isHumanTurn = b;
...
void GameScene::playerDrawCard(int id)
{
...
void GameScene::AIDrawCard(int id)
{
	if (haveSound) { AudioEngine::play2d("Sounds/drawCard.mp3", false, 1.0); }
...
void GameScene::humanTurn()
{
	...
	if (!gameOver)
	{
		...
		MoveAttListener->onTouchBegan = CC_CALLBACK_2(GameScene::onTouchBeganMATT, this);
		MoveAttListener->onTouchMoved = CC_CALLBACK_2(GameScene::onTouchMovedMATT, this);
		MoveAttListener->onTouchEnded = CC_CALLBACK_2(GameScene::onTouchEndedMATT, this);

		Director::getInstance()->getEventDispatcher()->addEventListenerWithFixedPriority(MoveAttListener, 1);
	}
}

void GameScene::AITurn()
{
	...
	if (AICardLib.size() <= 0)
	{
		...
		scheduleOnce(schedule_selector(GameScene::scheduleCallBack), 1.5);
	}
	else
	{
		if (AITurnCount<10)
			...
			if (AICanUseAtt)
			{
				...
				if (!haveChaoFeng)
				{
					...
					if (temp >= myHeroBlood)
					{
						...
						addChild(logLabel);
						scheduleOnce(schedule_selector(GameScene::scheduleCallBack3), 2.5);
					...
					for (int i = 0; i<ChaoFengAtt.size(); ++i)
					{
						...
						else if (haveChoose)
						{
							...
							for (int l = 0; l<numberOfAIChooseAtt.size(); ++l)
							{
								...
								attBeAttacked(numberOfAIChooseAtt[i], targetKB, false);
							}
						}
					}
...
void GameScene::changHeroBlood(bool isMyHero, int attNum)
{
	if (attNum<0)
	{
	...
	if (isMyHero)
	{
		...
		if (newBlood <= 0)
		{
			...
			scheduleOnce(schedule_selector(GameScene::ETGscheduleCallBack), 2.5);
		}
	}
	else
	{
...
void GameScene::endThisGame(bool isWin)
{
	gameOver = true;
...
void GameScene::scheduleCallBack3(float delta)
{
	removeChildByTag(90);
	removeChildByTag(90);
	removeChildByTag(90);
...
void GameScene::useAIDama(Card &C)
{
...
void GameScene::attBeAttacked(int attId, int blood, bool isMyAtt)
{
	if (blood<0)
	{
		if (haveSound) { AudioEngine::play2d("Sounds/addLife.mp3", false, 0.8); }
...
void GameScene::AllMyAttBeAttacked(int blood)
{
...
void GameScene::AllAIAttBeAttacked(int blood)
{
...
bool GameScene::onTouchBegan(Touch *touch, Event *unused_ecent)
{
...
void GameScene::DMSelectEvent(int camp, int role, int scanBlood)
{
	DMScan = scanBlood;
	...
	{
		auto AButton = MenuItemImage::create("UI/SelectTarget.png", "UI/SelectTarget.png", CC_CALLBACK_1(GameScene::DMmenuCallback, this));
		auto BButton = MenuItemImage::create("UI/SelectTarget.png", "UI/SelectTarget.png", CC_CALLBACK_1(GameScene::DMmenuCallback, this));
	...
	else if (camp == 1 && role == 1)
	{
		...
		{
			auto Button = MenuItemImage::create("UI/SelectTarget.png", "UI/SelectTarget.png", CC_CALLBACK_1(GameScene::DMmenuCallback2, this));
	...
	else if (camp == 2 && role == 1)
	{
		...
		{
			auto Button = MenuItemImage::create("UI/SelectTarget.png", "UI/SelectTarget.png", CC_CALLBACK_1(GameScene::DMmenuCallback3, this));
			Button->setPosition(S.attSprite->getPosition());
			Button->setTag(i);
			Menu->addChild(Button);
...
vector<int> GameScene::AISelectCard(bool canUseAttCard)
{
	AIEHandCard.clear();
	//初始化AI可用的卡组
	...
	if (AIEHandCard.size() != 0)
	{
		...
		of << inTrainData;
		of.close();
		system("svm-predict test.txt model.txt out.txt");

		fstream fs("out.txt"); string s;
		fs >> s;
...
bool GameScene::onTouchBeganMATT(Touch *touch, Event *unused_ecent)
{
...
void GameScene::onTouchMovedMATT(Touch *touch, Event *unused_ecent)
{
...
void GameScene::onTouchEndedMATT(Touch *touch, Event *unused_ecent)
{
...
void GameScene::ETGscheduleCallBack(float delta)
{
	if (myHeroBlood>0)
	{
		endThisGame(true);
